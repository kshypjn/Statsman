...
def score():
    ...
    if con.is_connected():
            ...
            if player[2] == home:
                ...
                if player[1] > 0:
                    for r in range(len(player) - 1):
                        w = Label(tv, width=8, text=player[r], fg='pink')
        ...
        s = str(f) + "\t\t-\t\t" + str(f2)
        print(s)
        Final = Label(tv, text=s, width=45)
        ...
        if f>f2:
            ...
            res = "Match Drawn!"
        result = Label(tv,text=res,width=25,bg='#3CB371')
        result.grid(row=k + 8, column=0, columnspan=4)


hometeam = Label(tv, text="Home Team:", width=14, bg="#3CB371")
ihome = Entry(tv, width=14)
awayteam = Label(tv, text="Away Team:", width=14, bg="#3CB371")
